...
#${FIND} . -regextype posix-extended -type f ! -regex "${EXCLUDE_FILTER}" -exec sh -c '${1} --in-place --expression "s/LispSkeleton/${2}/g" --expression "s/lisp-skeleton/${3}/g" --expression "s/lisp_skeleton/${4}/g" "${5}"' '_' "${SED}" "${NAME}" "${DASH}" "${UNDERSCORE}" '{}' \;
${FIND} . -regextype posix-extended -type f -regex "${INCLUDE_FILTER}" -exec sh -c '${1} --in-place --expression "s/LispSkeleton/${2}/g" --expression "s/lisp-skeleton/${3}/g" --expression "s/lisp_skeleton/${4}/g" "${5}"' '_' "${SED}" "${NAME}" "${DASH}" "${UNDERSCORE}" '{}' \;
# shellcheck disable=SC1117
${SED} --in-place --expression "s/bin\/ls/bin\/${INITIALS}/g" README.md Dockerfile
